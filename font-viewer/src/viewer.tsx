...
const SCRIPT_TESTS: Array<{
  ...
  { id: "latin", label: "Latin", codepoints: [0x0041, 0x0061, 0x005a, 0x007a] },
  {
    id: "cyrillic",
    label: "Cyrillic",
    codepoints: [0x0410, 0x0430, 0x042f, 0x044f, 0x0401, 0x0451],
  }, // А а Я я Ё ё
  ...
  { id: "thai", label: "Thai", codepoints: [0x0e01, 0x0e2e] }, // ก ฮ
  {
    ...
    label: "Japanese (Kana)",
    codepoints: [0x3042, 0x3093, 0x30a2, 0x30f3],
  }, // あ ん ア ン
...
const SAMPLES: Sample[] = [
  ...
  {
    id: "latin",
    label: "English (all letters)",
    text: "The quick brown fox jumps over the lazy dog.",
  },
  {
    id: "cyrillic",
    label: "Russian pangram",
  ...
  },
  {
    id: "arabic",
    label: "Arabic sample",
    text: "صِفْ خَلْفَ نَجْدٍ شَجَرًا يَزْهُو بِحُسْنٍ.",
  },
  {
    id: "devanagari",
    label: "Hindi sample",
    text: "प्यारे बच्चे जल्दी उठकर खूब पढ़ाई करें।",
  },
  { id: "thai", label: "Thai sample", text: "เป็นมนุษย์สุดประเสริฐเลิศคุณค่า" },
  {
    id: "hiragana_katakana",
    label: "Japanese sample",
    text: "いろはにほへと ちりぬるを / イロハニホヘト チリヌルヲ",
  },
  {
    id: "hangul",
    label: "Korean sample",
...
function tryParseOpenType(buffer: ArrayBuffer): {
  font?: Font;
  error?: string;
} {
  try {
...
function detectScriptsFromFont(
  font?: Font,
): Array<{ id: ScriptId; label: string; confidence: "high" | "medium" }> {
  if (!font) return [];
  ...
  for (const s of SCRIPT_TESTS) {
    const hits = s.codepoints.reduce(
      (acc, cp) => acc + (cps.has(cp) ? 1 : 0),
      0,
    );
    if (hits === 0) continue;
    supported.push({
      id: s.id,
      label: s.label,
      confidence: hits === s.codepoints.length ? "high" : "medium",
    });
  }
  return supported;
}
...
function pickSamplesForScripts(scripts: Array<{ id: ScriptId }>): Sample[] {
  ...
  return [
    ...
    },
    {
      id: "latin",
      label: "Symbols",
      text: "0123456789 !@#$%^&*() []{} <>=+-_— “quotes” ‘apostrophes’",
    },
  ];
}

function App(props: { viewerProps: ViewerProps }) {
  const { viewerProps } = props;
  const [family, setFamily] = useState(() =>
    randomFamilyName(viewerProps.fileName),
  );
  const [fontFaceStatus, setFontFaceStatus] = useState<
  ...
  const meta = useMemo(() => {
    if (!font) return null;
    const names =
      (font as unknown as { names?: Record<string, Record<string, string>> })
        .names ?? {};
      ...
      <div style={panelStyle}>
        {(fontFaceStatus === "error" || parseError) && (
          ...
          >
            <div style={{ fontWeight: 650, marginBottom: 6 }}>
              Some capabilities are limited for this font
            </div>
            {fontFaceError && (
            ...
            <div style={{ fontSize: 12, opacity: 0.75, marginTop: 8 }}>
              If this is a WOFF2 or font collection, preview may still work even
              when coverage detection can’t parse it.
            </div>
          </div>
        )}
        ...
        <div style={cardStyle}>
          ...
          >
            ...
            <div style={{ fontSize: 12, color: "var(--fg-muted)" }}>
              Weights/styles may be synthesized if the font doesn’t provide
              them.
            </div>
          </div>
          <div
          ...
          >
            {previewRows.map((row) => (
              <div
                key={`${row.label}-${row.weight}-${row.italic ? "i" : "n"}`}
        ...
        <div style={cardStyle}>
          <div style={{ fontSize: 13, fontWeight: 650, marginBottom: 8 }}>
            Detected scripts / locale hints
          </div>
          {scripts.length === 0 ? (
            <div style={{ fontSize: 12, color: "var(--fg-muted)" }}>
              No script coverage detected (or parser couldn’t read cmap). You
              can still preview the font above.
            </div>
          ) : (
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {scripts.map((s) => (
                <span
                  ...
                  title={
                    s.confidence === "high"
                      ? "All test glyphs present"
                      : "Some test glyphs present"
                  }
                >
...
export async function mountViewer(
  ...
  reactRoot.render(<App viewerProps={props} />);
}
